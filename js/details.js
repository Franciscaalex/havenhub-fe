(function () {
  ...
  function renderGallery(property) {
    const photos = property.photos && property.photos.length > 0 ? property.photos : [property.img || ''];
      ...
      <div class="details-container">
        ...
        <div class="details-content-grid">
          ...
          <div class="details-main-info">
            ...
            <div class="details-price">$${(property.price ?? 0).toLocaleString()}/mo</div>

            <div class="details-badges">
              <span class="details-badge"><img src="Bed.png" alt="" /> ${property.beds ?? 0} Bedroom${property.beds === 1 ? "" : "s"}</span>
              <span class="details-badge"><img src="Shower.png" alt="" /> ${property.baths ?? 0} Bathroom${property.baths === 1 ? "" : "s"}</span>
            </div>

            <!-- Interactivity Buttons -->
            <div class="details-actions" style="margin-top: 20px; display: flex; gap: 12px;">
              <button id="savePropertyBtn" class="action-btn ${property.isSaved ? 'is-saved' : ''}">
                Save Property
              </button>
              <button id="messageLandlordBtn" class="action-btn primary">
                Message Landlord
              </button>
            </div>

            <!-- Key Highlights Box -->
            <div class="details-highlights" style="margin-top: 20px;">
  ...
  // --------------------------------
  // Event Actions Routing To v1 Paths
  // --------------------------------
  function wireInteractionButtons() {
    document.getElementById('savePropertyBtn')?.addEventListener('click', async (e) => {
      const btn = e.currentTarget;
      const isSaved = btn.classList.toggle('is-saved');
      try {
        if (isSaved) {
          await window.api.post(`/api/v1/properties/${propertyId}/save`);
        } else {
          await window.api.delete(`/api/v1/properties/${propertyId}/save`);
        }
      } catch (err) {
    ...
    document.getElementById('messageLandlordBtn')?.addEventListener('click', () => {
      window.location.href = `inquire.html?propertyId=${propertyId}`;
    });
  }
  ...
  function showNotFound() {
    root.innerHTML = `<div class="details-container"><div class="empty-state"><h3>Property not found</h3><a href="index.html">Back to Browse</a></div></div>`;
  }

  // --------------------------------
  // Main Execution Execution Block
  // --------------------------------
  showLoading();

  // Explicitly utilizing production-aligned endpoints /api/v1/
  window.api.get(`/api/v1/properties/${propertyId}`)
    .then((property) => {
      if (!property) {
        showNotFound();
        return;
